...
function RepositoriesList() {
  ...
  return (
    <div className="container">
      <h1>My GitHub Repositories</h1>
      <Btns setRepositories={setRepositories} repositories={repositories} />
      <ul>
        {repositories.map(repo => (
          <li key={repo.id}>
            <Link to={`/repo/${repo.id}`}>{repo.name}</Link>
      ...
      <div className="pagination">
        <button className='dir' onClick={goToPreviousPage} disabled={currentPage === 1}><FaArrowLeft/></button>
        <button className='dir1' onClick={goToNextPage}><FaArrowRight/></button>
      </div>
    </div>
  );
}
